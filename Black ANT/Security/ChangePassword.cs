...
namespace Security
{
    ...
    {
        private string _CurrentSHA512HashPass = "";
        private string _NewSHA512HashPass = "";
        private PasswordStrength pwdStrength = null;
        private LinearGradientBrush brush = null;

        private bool _CurrentPassCorrect = false;
        public bool CurrentPassCorrect
        {
            get { return _CurrentPassCorrect; }
            set
            {
                _CurrentPassCorrect = value;
                if (CurrentPassCorrect && ReenterPassCorrect)
                    btnOk.Enabled = true;
                else btnOk.Enabled = false;
            }
        }

        private bool _ReenterPassCorrect = false;
        public bool ReenterPassCorrect
        {
            get { return _ReenterPassCorrect; }
            set
            {
                _ReenterPassCorrect = value;
                if (CurrentPassCorrect && ReenterPassCorrect)
                    btnOk.Enabled = true;
                else btnOk.Enabled = false;
            }
        ...
        public string NewSHA512HashPass { get { return _NewSHA512HashPass; } }

        
        public ChangePassword(string currentSHA512HashPass)
        {
            InitializeComponent();

            _CurrentSHA512HashPass = currentSHA512HashPass;

            pwdStrength = new PasswordStrength();
            pwdStrength.SetPassword(" ");
            UpdateControl();

            txtCurrentPass.Leave += (source, e) => { checkCurrentPass(txtCurrentPass.Text); };
            txtNewPass.TextChanged += (source, e) => { SetPassword(txtNewPass.Text); checkReenterPass(); };
            txtReenterPass.TextChanged += (source, e) => { checkReenterPass(); };
        ...
        protected void tableLayoutPanel_Paint(object sender, PaintEventArgs e)
        {
            if (brush != null)
            {
                e.Graphics.FillRectangle(brush, this.tableLayoutPanel.ClientRectangle);
            }
        }
        ...
        private void UpdateControl()
        {
            ...
            lblStrength.Text = pwdStrength.GetPasswordStrength();
            int x1 = score * 4;
            int x2 = x1 + 99;
            if (x2 >= 500) x2 = 499;
        ...
        private void checkReenterPass()
        {
            if (txtReenterPass.Text == txtNewPass.Text && txtReenterPass.Text != string.Empty)
            {
                pbStateReenterPass.CurrentStatus = PictureBoxStatus.ImageState.Loaded;
                ReenterPassCorrect = true;
            }
            else
            {
                pbStateReenterPass.CurrentStatus = PictureBoxStatus.ImageState.NotLoad;
                ReenterPassCorrect = false;
            }
        }
        private void checkCurrentPass(string p)
        {
            if (p.GetSHA512HashCode() == _CurrentSHA512HashPass)
            {
                pbStateCurrentPass.CurrentStatus = PictureBoxStatus.ImageState.Loaded;
                CurrentPassCorrect = true;
            }
            else
            {
                pbStateCurrentPass.CurrentStatus = PictureBoxStatus.ImageState.NotLoad;
                CurrentPassCorrect = false;
            }
        }

        
        private void btnPassReveal_CheckedChanged(object sender, EventArgs e)
        {
            if(btnPassReveal.Checked)
            {
                btnPassReveal.Text = "℗";
        ...
        {
            this._NewSHA512HashPass = txtNewPass.Text.GetSHA512HashCode();
